...
class Update extends Base
{
    ...
    public function index(Request $request, int $targetId = 0)
    {
        ...
        if (!count($this->error)) {
            ...
            if (count($cleanVars) > 0) {
                $res = SettingLangModel::where("id", $targetId)->update($cleanVars);
            }

            # [result]
            if ($res) {
                LogAdminModel::log($request, "update", "setting_lang", $targetId);
                $this->response = [
                    "success" => true,
                ];
            }
    ...
    private function checking(array $params = [])
    {
        ...
        if (!empty($params["code"])) {
            if (SettingLangModel::where("code", $params["code"])
