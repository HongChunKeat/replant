...
class Create extends Base
{
    ...
    public function index(Request $request)
    {
        ...
        if (!count($this->error)) {
            ...
            if (count($cleanVars) > 0) {
                ...
                $res = SettingItemAttributeModel::create($cleanVars);
            }

            # [result]
            if ($res) {
                LogAdminModel::log($request, "create", "setting_item_attribute", $res["id"]);
                $this->response = [
                    "success" => true,
                ];
            }
    ...
    private function checking(array $params = [])
    {
        ...
        if (isset($params["item"]) && isset($params["attribute"])) {
            if (SettingItemAttributeModel::where(["item_id" => $params["item"], "attribute_id" => $params["attribute"]])->first()) {
