...
class Update extends Base
{
    ...
    protected $rule = [
        "uid" => "number|max:11",
        "upline_uid" => "number|egt:0|max:11",
        "remark" => "",
    ];
    ...
    public function index(Request $request, int $targetId = 0)
    {
        ...
        if (!count($this->error)) {
            ...
            if (count($cleanVars) > 0) {
                $res = NetworkSponsorModel::where("id", $targetId)->update($cleanVars);
            }

            # [result]
            if ($res) {
                LogAdminModel::log($request, "update", "network_sponsor", $targetId);
                $this->response = [
                    "success" => true,
                ];
            }
    ...
    private function checking(array $params = [])
    {
        ...
        if (!empty($params["uid"])) {
            ...
            if (NetworkSponsorModel::where("uid", $params["uid"])
        ...
        // check upline_uid
        if (!empty($params["upline_uid"]) && $params["upline_uid"] != 0) {
